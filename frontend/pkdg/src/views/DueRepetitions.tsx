...
const DueRepetitions = () => {
  ...
  if (dueRepetitions.length == 0) {
    return <div className="mt-5 flex justify-center">
      Brak powtórek na ten moment.
    </div>
  }

  return <div className="mt-5 p-5 flex flex-col items-center">
        ...
        <div className="flex flex-col p-4 w-full leading-normal">
            {
              ...
              <>
                ...
                <ul className="list-disc pl-6">
                  {dueRepetitions[0].word.examples.map(example => <li key={example.sentence}>{example.sentence}</li>)}
                </ul>
                <div className="flex flex-row mt-5 justify-center">
                  <button onClick={() => onRepetitionEvaluate(0, dueRepetitions[0].repetition)} className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">1</button>
                  <button onClick={() => onRepetitionEvaluate(1, dueRepetitions[0].repetition)} className="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:focus:ring-yellow-900">2</button>
                  <button onClick={() => onRepetitionEvaluate(2, dueRepetitions[0].repetition)} className="py-2.5 px-5 mr-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">3</button>
                  <button onClick={() => onRepetitionEvaluate(3, dueRepetitions[0].repetition)} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">4</button>
                  <button onClick={() => onRepetitionEvaluate(4, dueRepetitions[0].repetition)} className="focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900">5</button>
                  <button onClick={() => onRepetitionEvaluate(5, dueRepetitions[0].repetition)} className="focus:outline-none ml-2 text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">6</button>
                </div>
              </>
            }
            {
              ...
              <>
                ...
                <button onClick={onShowAnswerClick} className="cursor-pointer mt-2 text-white bg-blue-700 w-40 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                  Pokaż odpowiedź
                </button>
              </>
            }
